...
| 取消建议 | 订阅使用频率低且即将续费 | rarely + 7天 |
| 替换建议 | 物理资产折旧严重 | 低于原价10% + 有收藏供应商 |

提醒方式：应用内通知 + Web Push 浏览器推送（已实现）。
...
- [x] Web Push 浏览器推送通知
- [x] 仪表盘数据可视化（Recharts 饼图 + 折线图）
- [x] 通知页面（未读/全部筛选，批量操作）
- [x] 资产批量操作（批量修改状态、批量删除）
- [x] 采购建议（收藏供应商推荐、续费决策、折旧替换建议）
- [x] 删除资产关联影响提示
